...
class PacmanAgent(Agent):
    ...
    def minimax(self, state):
        ...
        for s in state.generatePacmanSuccessors():
            ...
            minimax = self.minimaxrec(s[0], 1, 0, s[1])
    ...
    def minimaxrec(self, state, player, dpt=0,lstPcmnMove = None):
        ...
        for s in successors:
            newState = s[0]
            minimax = self.minimaxrec(newState, self.getNextPlayer(player), dpt+1, s[1])
            if minimax != None:
                sol.append(minimax)
    ...
    def generateSuccessors(self, state, player, lastmove=None):
        if player == 0:
            ...
            # We are near a wall
            if len(nextStates) < 4:
                bumbedInWall = len([item for item in nextStates if item[1] == lastmove])
                print(lastmove)
                if bumbedInWall == 0:
